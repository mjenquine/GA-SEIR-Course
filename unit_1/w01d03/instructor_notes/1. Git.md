...
1. Go to a suitable location in the terminal (let's do `~/dev/`) and type `git clone ` and paste the ssh url (e.g.`git clone git@git.generalassemb.ly:mahuntington/asdfasdf.git`)
