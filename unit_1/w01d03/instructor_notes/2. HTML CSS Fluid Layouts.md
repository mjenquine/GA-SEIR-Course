...
- Navigate to today's `student_examples` folder
- open `index.html` and `main.css` you can use the command `atom .` (Atom) or `code .` (VS Code)
...
### Floats were and are a hack to achieve layouts that are better suited with newer CSS options
#### Devs had to use floats for a very long time in order to support IE
##### You'll likely see legacy code with floats, but if you don't have to, don't write new legacy code! Learn the newer way to do things
