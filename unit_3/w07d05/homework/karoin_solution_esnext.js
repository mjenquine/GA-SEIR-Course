...
const { make, colors_available, honk } = carObj
...
const isMinNegative = arr =>
  Math.min(...arr) > -1 ? 'min is positive' : 'min is negative'

// console.log(isMinNegative(arr1))
// console.log(isMinNegative(arr2))
...
// add let to block the scope of key to just the loop
for (let key of belongings) {
  // console.log('Timmy checks his satchel and finds', key)
}
belongings.push(key)
// console.log(
//   'Timmy now has they gold key',
//   belongings[belongings.length - 1] === 'a golden key'
// )
...
const sortOnlyStrigs = (...args) => {
  const strings = args.filter(item => typeof item === 'string').sort()
  return (concatenate = strings.reduce(
