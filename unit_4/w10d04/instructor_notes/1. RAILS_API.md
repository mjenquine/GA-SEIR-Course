...
|Views|we used ejs (handlebars is another common one)| erb, BUT! We won't use it, when we're ready we'll jump into using React|part of MVC|
...
`rails db:create` creates your postgres database based on the name of your Rails app. Your terminal output should looks so

```
Created database 'todo_app_api_development'
Created database 'todo_app_api_test'
```
...
**Note**: unlike many other rails `db` commands `dbconsole` is **not** separated by a colon.

This will open up our postgres CLI already connected *already* connected to our `todo_app_api_development` database. You can do the same thing by typing `psql -d todo_app_api_development` but Rails gives us a handy shortcut
...
First, create a migration in Terminal. This migration will allow us to make a **table** in our database. To run a migration, run `rails generate migration CreateTodos`
...
```
== 20190521140815 CreateTodos: migrating ======================================
-- create_table(:todos)
   -> 0.0228s
== 20190521140815 CreateTodos: migrated (0.0229s) =============================
```
...
```ruby
ActiveRecord::Schema.define(version: 2019_05_21_140815) do
...
todo_app_api_development=# SELECT * FROM todos;
 id | title | completed 
----+-------+-----------
(0 rows)

todo_app_api_development=# 
```
...
Now's your turn. I'll give you some time to work until this point. Build your own `todo_app_api` referring to the notes above. Create a new rails api, the matching postgres databases and create *and* run your migration.
...
* In the migration file, a todo table should have columns for title, author, and content (all strings)
* Run the migration.
...
Back to our `todo_app_api` where we are making the **Todos**:
...
```ruby
todo = Todo.create(title: "Something", completed: false)
```
